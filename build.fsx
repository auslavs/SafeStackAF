...
let serverPath = Path.getFullName "./src/ServerAF"
...
let func args workingDir =
    let npmPath =
        ...
        | Some path -> path
        | None ->
            "func was not found in path. Please install the Azure Functions Core Tools and make sure it's available from your path. " +
            "See https://docs.microsoft.com/en-us/azure/azure-functions/functions-run-local for more info"
            |> failwith
...
Target.create "Azure" (fun _ ->
    ...
    deployment
    |> Deploy.execute "SafeStackAF" Deploy.NoParameters
    |> ignore
)
